...
import { getSeedData } from '../config';
...
export async function initializeDatabase(): Promise<void> {
  ...
  for (const [tableName, tableData] of Object.entries(schemaConfig.tables)) {
    ...
    const allConstraints: string[] = [...constraints];
...
async function seedDataFromConfig(db: any): Promise<void> {
  ...
  for (let i = 0; i < seedData.orders.length; i++) {
    ...
    await db.run('INSERT INTO orders (id, customer_id, item, amount, created_at) VALUES (?, ?, ?, ?, ?)', 
      i + 1, order.customer_id, order.item, order.amount, order.created_at);
  }
